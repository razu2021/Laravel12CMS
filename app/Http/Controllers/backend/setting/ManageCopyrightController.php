...
class ManageCopyrightController extends Controller
{
    ...
    public function active($id,$slug){
        $active = Copyright::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status' => 1,
        ]);
    ...
    public function deactive($id,$slug){
        $active = Copyright::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status' => 0,
        ]);
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Copyright::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Copyright::whereIn('id',$ids)->where('public_status',0)->update([
                'public_status'=>1,
            ]);
        ...
        if($action === 'InActive'){
            $categorys = Copyright::whereIn('id',$ids)->where('public_status',1)->update([
                'public_status'=>0,
            ]);
        }
