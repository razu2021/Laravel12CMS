...
class SiteAddressController extends Controller
{
    ...
    public function update(Request $request){
        ...
        $id = $request->id;
        // ----- insert record into database 
        $update = SiteAddress::where('id',$id)->where('slug',$slug)->update([
            'type'=>$request->type,
            'address'=>$request->address,
            'title'=>$request->title,
            'description'=>$request->description,
    ...
    public function active($id,$slug){
        $active = SiteAddress::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status' => 1,
        ]);
    ...
    public function deactive($id,$slug){
        $active = SiteAddress::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status' => 0,
        ]);
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = SiteAddress::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = SiteAddress::whereIn('id',$ids)->where('public_status',0)->update([
                'public_status'=>1,
            ]);
        ...
        if($action === 'InActive'){
            $categorys = SiteAddress::whereIn('id',$ids)->where('public_status',1)->update([
                'public_status'=>0,
            ]);
        }
